-- DMS ERP V14.1 — UPDATE-ONLY SCRIPT (SAFE FOR LIVE DATA)

-- drop stale old-signature functions (prevents "could not choose best candidate")
...
drop function if exists post_voucher(text, text, text, date, date, date, uuid, uuid, text, numeric, text, text, jsonb, boolean, uuid, text);
drop function if exists post_voucher(text, text, text, date, date, date, uuid, uuid, text, numeric, text, text, jsonb, boolean, uuid, text, text);
drop function if exists edit_voucher(uuid, text, date, date, date, uuid, text, numeric, text, text, text, jsonb, uuid);
drop function if exists edit_voucher(uuid, text, date, date, date, uuid, text, numeric, text, text, text, jsonb, uuid, text);
drop function if exists set_doc_flag(uuid, text, boolean);
drop function if exists avg_weight_profile(uuid);
drop function if exists rcjw_no(text, date);
drop function if exists last_updated_status();
drop function if exists open_documents();
drop function if exists production_trend(date, date);

-- new columns/tables this release (safe, additive)
alter table vouchers add column if not exists location_id uuid references location(id);
alter table vouchers add column if not exists free_ledger text;
alter table vouchers add column if not exists rec_hold boolean default false;
alter table vouchers add column if not exists vehicle_no text;
alter table vouchers add column if not exists grn boolean default false;
alter table vouchers add column if not exists scrap_slip_wt numeric;
alter table part add column if not exists cumulative_group text;
alter table part add column if not exists lb_price numeric default 0;
alter table voucher_lines add column if not exists pkg_count numeric default 0;
alter table voucher_lines add column if not exists lot_alloc jsonb;
alter table production_rows add column if not exists lot_alloc jsonb;
...
alter table part_price add column if not exists lb_price numeric default 0;
...
alter table user_module_rights add column if not exists can_edit boolean default false;
alter table user_module_rights add column if not exists can_markdel boolean default false;
alter table user_module_rights add column if not exists can_markedit boolean default false;

create sequence if not exists production_code_seq;
alter table production_log add column if not exists log_code text;
alter table production_log alter column log_code set default lpad(nextval('production_code_seq')::text, 6, '0');
do $bf$
declare r record;
begin
  for r in select id from production_log where log_code is null order by created_at, id loop
    update production_log set log_code = lpad(nextval('production_code_seq')::text, 6, '0') where id = r.id;
  end loop;
end $bf$;

insert into bucket_map(voucher_type,from_bucket,to_bucket,note)
  values ('RESALE','MG','CUSTOMER','-MG (resale, no SO)')
...
create or replace function get_settings() returns table(key text, value text) as $$ select key,value from app_settings; $$ language sql;
...
drop function if exists db_storage_total() cascade;
...
drop function if exists db_storage_by_table() cascade;
...
create or replace function next_ledger_code(p_type text) returns text as $$
...
create or replace function get_price(p_part uuid, p_ledger uuid, p_type text, p_date date)
returns numeric as $$
...
drop view if exists stock_var cascade;
...
create or replace function check_stock_loc(p_part uuid, p_loc uuid, p_bucket text) returns numeric as $$
...
create or replace function check_stock(p_part uuid, p_bucket text) returns numeric as $$
  select coalesce((select sum(bal) from stock_balance where part_id=p_part and bucket=p_bucket),0); $$ language sql;

drop view if exists stock_explode cascade;
...
drop function if exists cumulative_groups() cascade;
...
drop view if exists stock_diagnostics cascade;
...
drop view if exists jobout_by_source cascade;
...
drop function if exists get_recon_grid() cascade;
create or replace function get_recon_grid() returns table(part_id uuid, part_code text, part_name text, bucket text, system_qty numeric) as $$
...
create or replace function post_reconciliation(p_date date, p_user text, p_rows jsonb) returns int as $$
...
create or replace function next_lot_no(p_part uuid, p_ledger uuid) returns text as $$
...
create or replace function lot_balance(p_lot uuid, p_bucket text) returns numeric as $$
...
drop function if exists available_lots(uuid, text) cascade;
create or replace function available_lots(p_part uuid, p_bucket text) returns table(lot_id uuid, lot_no text, ledger text, available numeric, origin_date date) as $$
...
drop view if exists lot_wise_stock cascade;
...
create or replace function next_voucher_idcode(p_type text) returns text as $$
declare pre text; n int; begin
  pre := case p_type
...
create or replace function valid_thru_5th(p_date date) returns date as $$
...
drop view if exists order_fulfilment cascade;
...
drop view if exists open_orders cascade;
...
drop view if exists dc_fulfilment cascade;
...
drop view if exists open_dcs cascade;
create view open_dcs as select * from dc_fulfilment where pending_qty>0;

create or replace function log_audit(p_action text, p_user text, p_details text) returns void as $$ insert into audit_log(action,app_user,details) values(p_action,p_user,p_details); $$ language sql;

drop function if exists price_pending() cascade;
create or replace function price_pending() returns table(id uuid, voucher_no text, voucher_date date, ledger_id uuid) as $$
...
create or replace function approve_price(p_id uuid) returns void as $$ update vouchers set price_approved='OK' where id=p_id; $$ language sql security definer;

drop function if exists rec_copy_pending() cascade;
create or replace function rec_copy_pending() returns table(id uuid, voucher_type text, voucher_no text, voucher_date date) as $$
...
create or replace function approve_rec_copy(p_ids uuid[]) returns int as $$
...
drop function if exists rec_copy_holds() cascade;
...
create or replace function mark_record(p_id uuid, p_mark text, p_reason text, p_user text, p_role text) returns jsonb as $$
...
drop function if exists marked_requests() cascade;
create or replace function marked_requests() returns table(id uuid, voucher_type text, voucher_no text, delete_requested boolean, modify_requested boolean, request_reason text, requested_by text) as $$
...
create or replace function resolve_mark(p_id uuid, p_mark text, p_action text, p_admin text) returns void as $$
begin
...
drop function if exists list_vouchers(text, boolean) cascade;
create or replace function list_vouchers(p_type text, p_include_pending boolean default true) returns table(
...
create or replace function set_doc_flag(p_id uuid, p_field text, p_val boolean, p_user text default null) returns void as $$
...
drop function if exists part_ledger(uuid, text, date, date) cascade;
...
drop function if exists scrap_report(date, date) cascade;
create or replace function scrap_report(p_from date, p_to date) returns table(part_code text, part_name text, produced numeric, scrap_wt_pc numeric, total_scrap numeric) as $$
...
drop function if exists last_updated_status() cascade;
create or replace function last_updated_status() returns table(voucher_type text, last_at timestamptz, cnt bigint) as $$
...
create or replace function post_production(p_date date, p_shift text, p_sup1 text, p_sup2 text, p_user text, p_rows jsonb, p_downtime jsonb, p_quality jsonb, p_location uuid default null)
returns uuid as $$
...
create or replace function admin_list_users() returns table(id uuid, username text, role text, access_modules text, weight_check boolean, valid_thru_edit boolean, active boolean, last_login timestamptz) as $$
...
create or replace function admin_save_user(p_id uuid, p_username text, p_password text, p_role text, p_access text, p_weight boolean, p_valid_edit boolean, p_active boolean default true) returns uuid as $$
...
create or replace function admin_delete_user(p_id uuid) returns void as $$ delete from app_users where id=p_id and username<>'admin'; $$ language sql security definer;

drop function if exists get_module_rights(uuid) cascade;
create or replace function get_module_rights(p_user uuid) returns table(module text, can_view boolean, can_create boolean, can_approve boolean, can_edit boolean, can_markdel boolean, can_markedit boolean) as $$
...
create or replace function set_module_right(p_user uuid, p_module text, p_view boolean, p_create boolean, p_approve boolean, p_edit boolean default false, p_markdel boolean default false, p_markedit boolean default false) returns void as $$
...
create or replace function admin_save_opening(p_rows jsonb) returns int as $$
...
drop function if exists get_checkbox_perms(uuid) cascade;
...
drop function if exists list_part_groups() cascade;
...
drop function if exists production_layout() cascade;
...
drop function if exists recent_transactions(int) cascade;
...
drop function if exists open_documents() cascade;
...
drop function if exists opening_grid() cascade;
...
drop function if exists reconcile_stock() cascade;
...
drop function if exists audit_negatives() cascade;
...
drop function if exists engine_health() cascade;
...
drop function if exists get_weight_profile(uuid, uuid, date) cascade;
...
drop function if exists list_voucher_enabled() cascade;
...
drop function if exists list_part_prices(text) cascade;
...
drop function if exists dashboard_metrics() cascade;
...
drop function if exists get_column_config(text) cascade;
...
drop function if exists list_vouchers_full(text) cascade;
...
drop function if exists list_supervisors() cascade;
...
drop function if exists list_defect_types() cascade;
...
drop function if exists parts_for_vendor(uuid) cascade;
...
drop function if exists parts_for_customer(uuid) cascade;
...
drop function if exists open_pos_for(uuid, uuid) cascade;
...
drop function if exists open_sos_for(uuid, uuid) cascade;
...
drop function if exists open_dcs_for(uuid, uuid) cascade;
...
drop function if exists avg_weight_profile(uuid) cascade;
...
drop function if exists overdue_dcjw() cascade;
...
drop function if exists price_pending_full() cascade;
...
drop function if exists price_pending_lines(uuid) cascade;
...
drop function if exists part_ledger_full(uuid, text, date, date) cascade;
...
drop function if exists part_vendor_summary(uuid, date, date) cascade;
...
drop function if exists vendor_part_summary(uuid, date, date, uuid) cascade;
...
drop function if exists vendor_doc_lines(uuid, date, date, uuid) cascade;
...
drop function if exists part_ledger_group(text, text, date, date) cascade;
...
drop function if exists stock_ledger_full(uuid, text, text, date, date) cascade;
...
drop function if exists voucher_counts() cascade;
...
drop function if exists audit_log_recent(int) cascade;
...
drop function if exists stuck_stock(int) cascade;
...
drop function if exists stock_valuation() cascade;
...
drop function if exists pending_approvals() cascade;
...
drop function if exists production_vs_target() cascade;
...
drop function if exists closed_periods() cascade;
...
drop function if exists stock_transfers(int) cascade;
...
drop function if exists list_locations(boolean) cascade;
...
drop function if exists location_buckets(uuid) cascade;
...
drop function if exists stock_summary_loc(uuid) cascade;
...
drop function if exists open_variant_dcs(text) cascade;
...
drop function if exists stock_summary_cumulative() cascade;
...
drop function if exists list_production_books() cascade;
...
drop function if exists production_summary(date, date) cascade;
...
drop function if exists production_summary_machines(date, date) cascade;
